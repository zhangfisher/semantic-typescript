...
export let event: EventFunction = <E extends Window | Document | HTMLElement, K extends KeyOfEventMap<E>, V = ValueOfEventMap<E, K>>(argument1: Window | Document | HTMLElement | Iterable<HTMLElement>, argument2: K | Iterable<K>): Semantic<V> => {
    if (isWindow(argument1)) {
        let target: Window = argument1;
        if (isString(argument2)) {
            let key: K = argument2 as K & keyof WindowEventMap;
            return new Semantic<V>((accept: Consumer<V> | BiConsumer<V, bigint>, interrupt: Predicate<V> | BiPredicate<V, bigint>): void => {
                try {
                    let index: bigint = 0n;
                    let listener: (event: Event) => void = (event: Event): void => {
                        if (interrupt(event as V, index)) {
                            target.removeEventListener(key, listener);
                        } else {
                            accept(event as V, index);
                            index++;
                        }
                    };
    ...
    if (isDocument(argument1)) {
        let target: Document = argument1;
        if (isString(argument2)) {
            let key: K = argument2 as K & keyof DocumentEventMap;
            return new Semantic<V>((accept: Consumer<V> | BiConsumer<V, bigint>, interrupt: Predicate<V> | BiPredicate<V, bigint>): void => {
                try {
                    let index: bigint = 0n;
                    let listener: (event: Event) => void = (event: Event): void => {
    ...
    if (isIterable(argument1)) {
        let targets: Iterable<HTMLElement> = argument1;
        if (isString(argument2)) {
            let key: K = argument2 as K & keyof HTMLElementEventMap;
            return new Semantic<V>((accept: Consumer<V> | BiConsumer<V, bigint>, interrupt: Predicate<V> | BiPredicate<V, bigint>): void => {
                try {
                    let index: bigint = 0n;
