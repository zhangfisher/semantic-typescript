...
import { BigIntStatisticsSymbol, BinaryNodeSymbol, CollectableSymbol, CollectorsSymbol, LinearNodeSymbol, NodeSymbol, 
    NumericStatisticsSymbol, OptionalSymbol, OrderedCollectableSymbol, 
    RedBlackNodeSymbol, 
    SemanticMapSymbol, SemanticSymbol, StatisticsSymbol, UnorderedCollectableSymbol, WindowCollectableSymbol } from "./symbol";
...
export let isSemanticMap: (target: unknown) => target is SemanticMap<unknown, unknown> = (target: unknown): target is SemanticMap<unknown, unknown> => {
    if (isObject(target)) {
        return Reflect.get(target, "SemanticMap") === SemanticMapSymbol;
    }
    return false;
};
export let isHashMap: (target: unknown) => target is HashMap<unknown, unknown> = (target: unknown): target is HashMap<unknown, unknown> => {
    if (isObject(target)) {
        return Reflect.get(target, "HashMap") === SemanticMapSymbol;
    }
    return false;
};
export let isHashSet: (target: unknown) => target is HashSet<unknown> = (target: unknown): target is HashSet<unknown> => {
    if (isObject(target)) {
        return false;
    }
    return false;
};
export let isNode: (target: unknown) => target is Node<unknown, any> = (target: unknown): target is Node<unknown, any> => {
    if (isObject(target)) {
        return Reflect.get(target, "Node") === NodeSymbol;
    }
    return false;
};
export let isLinearNode: (target: unknown) => target is LinearNode<unknown> = (target: unknown): target is LinearNode<unknown> => {
    if (isObject(target)) {
        return isNode(target) && Reflect.get(target, "LinearNode") === LinearNodeSymbol;
    }
    return false;
};
export let isBinaryNode: (target: unknown) => target is BinaryNode<unknown, any> = (target: unknown): target is BinaryNode<unknown, any> => {
    if (isObject(target)) {
        return isNode(target) && Reflect.get(target, "BinaryNode") === BinaryNodeSymbol;
    }
    return false;
};
export let isRedBlackNode: (target: unknown) => target is BinaryNode<unknown, any> = (target: unknown): target is BinaryNode<unknown, any> => {
    if (isObject(target)) {
        return isBinaryNode(target) && Reflect.get(target, "RedBlackNode") === RedBlackNodeSymbol;
    }
    return false;
};
export let isTree: (target: unknown) => target is Node<unknown, any> = (target: unknown): target is Node<unknown, any> => {
    if (isObject(target)) {
        return isNode(target) && (isLinearNode(target) || isBinaryNode(target));
    }
    return false;
};
export let isBinaryTree: (target: unknown) => target is Node<unknown, any> = (target: unknown): target is Node<unknown, any> => {
    if (isObject(target)) {
        return isNode(target) && isBinaryNode(target);
    }
    return false;
};
export let isRedBlackTree: (target: unknown) => target is Node<unknown, any> = (target: unknown): target is Node<unknown, any> => {
    if (isObject(target)) {
