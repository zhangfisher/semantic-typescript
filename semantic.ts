type Invalid<T> = T extends null | undefined ? T : never;
type Valid<T> = T extends null | undefined ? never : T;
type MaybeInvalid<T> = T | null | undefined;

let validate: <T>(t: MaybeInvalid<T>) => t is T = <T>(t: T | null | undefined): t is T => {
    return t !== null && t !== (void 0);
};

let invalidate: <T>(t: MaybeInvalid<T>) => t is (null | undefined) = <T>(t: T | null | undefined): t is (null | undefined) => {
    return t === null || t === undefined;
};

let isBoolean: (t: unknown) => t is boolean = (t: unknown): t is boolean => {
    return typeof t === "boolean";
};
let isString: (t: unknown) => t is string = (t: unknown): t is string => {
    return typeof t === "string";
};
let isNumber: (t: unknown) => t is number = (t: unknown): t is number => {
    return typeof t === "number";
};
let isFunction: (t: unknown) => t is Function = (t: unknown): t is Function => {
    return typeof t === "function";
};
let isObject: (t: unknown) => t is object = (t: unknown): t is object => {
    return typeof t === "object" && t !== null;
};
let isSymbol: (t: unknown) => t is symbol = (t: unknown): t is symbol => {
    return typeof t === "symbol";
};
let isBigint: (t: unknown) => t is bigint = (t: unknown): t is bigint => {
    return typeof t === "bigint";
};
let isIterable: (t: unknown) => t is Iterable<unknown> = (t: unknown): t is Iterable<unknown> => {
    if (isObject(t)) {
...
let useCompare: <T>(t1: T, t2: T) => number = <T>(t1: T, t2: T): number => {
...
let useRandom: <T = number | bigint>(index: T) => T = <T = number | bigint>(index: T): T => {
    if (isNumber(index)) {
        let x = Number(index);
...
let OptionalSymbol = Symbol.for("Optional");
let SemanticSymbol = Symbol.for("Semantic");
let CollectorsSymbol = Symbol.for("Collector");
let CollectableSymbol = Symbol.for("Collectable");
let OrderedCollectableSymbol = Symbol.for("OrderedCollectable");
let UnorderedCollectableSymbol = Symbol.for("UnorderedCollectable");
let StatisticsSymbol = Symbol.for("Statistics");

let isOptional: (t: unknown) => t is Optional<unknown> = (t: unknown): t is Optional<unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "Optional") === OptionalSymbol;
    }
    return false;
};
let isSemantic: (t: unknown) => t is Semantic<unknown> = (t: unknown): t is Semantic<unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "Semantic") === SemanticSymbol;
    }
    return false;
};
let isCollector: (t: unknown) => t is Collector<unknown, unknown, unknown> = (t: unknown): t is Collector<unknown, unknown, unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "Collector") === CollectorsSymbol;
    }
    return false;
};
let isCollectable: (t: unknown) => t is Collectable<unknown> = (t: unknown): t is Collectable<unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "Collectable") === CollectableSymbol;
    }
    return false;
};
let isOrderedCollectable: (t: unknown) => t is OrderedCollectable<unknown> = (t: unknown): t is OrderedCollectable<unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "OrderedCollectable") === OrderedCollectableSymbol;
    }
    return false;
};
let isUnorderedCollectable: (t: unknown) => t is UnorderedCollectable<unknown> = (t: unknown): t is UnorderedCollectable<unknown> => {
    if (isObject(t)) {
        return Reflect.get(t, "UnorderedCollectable") === UnorderedCollectableSymbol;
    }
    return false;
};
let isStatistics: (t: unknown) => t is Statistics<unknown, number | bigint> = (t: unknown): t is Statistics<unknown, number | bigint> => {
    if (isObject(t)) {
...
let empty = <E>(): Semantic<E> => {
    return new Semantic<E>(() => { });
}

let fill = <E>(element: E | Supplier<E>, count: bigint): Semantic<E> => {
...
let from = <E>(iterable: Iterable<E>): Semantic<E> => {
...
let range: <N extends number | bigint>(start: N, end: N, step: N) => Semantic<N> = <N extends number | bigint>(start: N, end: N, step: N = (typeof start === 'bigint' ? 1n : 1) as N): Semantic<N> => {
...
abstract class Statistics<E, D extends number | bigint> extends OrderedCollectable<E> {
...
class NumericStatistics<E> extends Statistics<E, number> {
...
class BigIntStatistics<E> extends Statistics<E, bigint> {
